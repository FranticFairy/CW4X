var Constructor = function () {
    ...
    this.canBePerformed = function (action) {
        ...
        if ((actionTargetField.x === targetField.x) && (actionTargetField.y === targetField.y) ||
            ...
            if (ACTION_BUILD_INFRA_STREET.getBuildFields(action).length > 0) {
                return true;
            }
        }
        return false;
    ...
    this.getBuildFields = function (action) {
        ...
        for (var i = 0; i < targetFields.length; i++) {
            if (map.onMap(targetFields[i].x, targetFields[i].y)) {
                ...
                var buildables = ["PLAINS", "FOREST", "DESERT", "DESERT_PATH", "DESERT_FOREST", "SNOW", "SNOW_FOREST", "PLAINS_DESTROYED", "PLAINS_PLASMA", "DESTROYEDWELD", "DESERT_DESTROYEDWELD", "SNOW_DESTROYEDWELD"];
    ...
    this.getStepData = function (action, data) {
        ...
        var fields = ACTION_BUILD_INFRA_STREET.getBuildFields(action);
    ...
    this.perform = function (action) {
        ...
        animation.setEndOfAnimationCall("ACTION_BUILD_INFRA_STREET", "performPostAnimation");
        // move unit to target position
        unit.moveUnitAction(action);
        // disable unit commandments for this turn
        ACTION_BUILD_INFRA_STREET.postAnimationUnit = unit;
        //unit.setHasMoved(true);
        action.startReading();
        unit.setHasMoved(true);
        var x = action.readDataInt32();
        var y = action.readDataInt32();
        ACTION_BUILD_INFRA_STREET.postAnimationBuildPosX = x;
        ACTION_BUILD_INFRA_STREET.postAnimationBuildPosY = y;

    };
    this.performPostAnimation = function (postAnimation) {
        // unloading the units here :)
        var player = map.getCurrentPlayer();
        var X = ACTION_BUILD_INFRA_STREET.postAnimationBuildPosX;
        var Y = ACTION_BUILD_INFRA_STREET.postAnimationBuildPosY;
        map.replaceTerrain("STREET", X, Y, true, true);
        ...
        ACTION_BUILD_INFRA_STREET.postAnimationBuildPosX = -1;
        ACTION_BUILD_INFRA_STREET.postAnimationBuildPosY = -1;
    };
}

Constructor.prototype = ACTION;
var ACTION_BUILD_INFRA_STREET = new Constructor();
