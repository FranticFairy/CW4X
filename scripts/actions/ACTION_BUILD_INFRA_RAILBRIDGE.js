var Constructor = function () {
    ...
    this.canBePerformed = function (action) {
        ...
        if ((actionTargetField.x === targetField.x) && (actionTargetField.y === targetField.y) ||
            ...
            if (ACTION_BUILD_INFRA_RAILBRIDGE.getBuildFields(action).length > 0) {
                return true;
            }
        }
        return false;
    ...
    this.getStepData = function (action, data) {
        ...
        var fields = ACTION_BUILD_INFRA_RAILBRIDGE.getBuildFields(action);
    ...
    this.perform = function (action) {
        ...
        animation.setEndOfAnimationCall("ACTION_BUILD_INFRA_RAILBRIDGE", "performPostAnimation");
        // move unit to target position
        unit.moveUnitAction(action);
        // disable unit commandments for this turn
        ACTION_BUILD_INFRA_RAILBRIDGE.postAnimationUnit = unit;
        //unit.setHasMoved(true);
        action.startReading();
        var x = action.readDataInt32();
        var y = action.readDataInt32();
        unit.setHasMoved(true);
        ACTION_BUILD_INFRA_RAILBRIDGE.postAnimationBuildPosX = x;
        ACTION_BUILD_INFRA_RAILBRIDGE.postAnimationBuildPosY = y;

    };
    this.performPostAnimation = function (postAnimation) {
        // unloading the units here :)
        var player = map.getCurrentPlayer();
        var X = ACTION_BUILD_INFRA_RAILBRIDGE.postAnimationBuildPosX;
        var Y = ACTION_BUILD_INFRA_RAILBRIDGE.postAnimationBuildPosY;
        map.replaceTerrain("RAILBRIDGE", X, Y, true, true);
        ...
        ACTION_BUILD_INFRA_RAILBRIDGE.postAnimationBuildPosX = -1;
        ACTION_BUILD_INFRA_RAILBRIDGE.postAnimationBuildPosY = -1;
    };
}

Constructor.prototype = ACTION;
var ACTION_BUILD_INFRA_RAILBRIDGE = new Constructor();
